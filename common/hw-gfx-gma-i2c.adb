...
package body HW.GFX.GMA.I2C is
   ...
   begin
      ...
      if (GMBUS2 and GMBUS2_GMBUS_ACTIVE) /= 0 then
         Debug.Put_Line ("GMBUS is still active?");
         Registers.Write
      ...
      if not Success then
         Debug.Put_Line ("GMBUs still not ready, clearing interrupt, try again");
   ...
   begin
      ...
      if Success then
         Debug.Put_Line ("INIT_GMBUS success, perform i2c_read");
         Registers.Write
           (Register => GMBUS_Regs (1),
