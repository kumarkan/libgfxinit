...
package body HW.GFX.GMA.PLLs.DKL is
   ...
   type PLL_Regs_Record is record
      DKL_PLL_ENABLE          : Registers.Registers_Index;
   ...
   PLL_Regs : constant PLL_Regs_Array :=
      PLL_Regs_Array'
     (TCPLL1 =>
        ((if Config.Has_Old_Type_C_PLL_Enable
	  then Registers.MGPLL1_ENABLE
	  else Registers.PORTTC_PLL_ENABLE_1),
      ...
      TCPLL2 =>
        ((if Config.Has_Old_Type_C_PLL_Enable
	  then Registers.MGPLL2_ENABLE
	  else Registers.PORTTC_PLL_ENABLE_2),
      ...
      TCPLL3 =>
        ((if Config.Has_Old_Type_C_PLL_Enable
	  then Registers.MGPLL3_ENABLE
	  else Registers.PORTTC_PLL_ENABLE_3),
      ...
      TCPLL4 =>
        ((if Config.Has_Old_Type_C_PLL_Enable
	  then Registers.MGPLL4_ENABLE
	  else Registers.PORTTC_PLL_ENABLE_4),
         ...
         Registers.DKL_PLL_COLD_BIAS_4),
      TCPLL5 =>
        (Registers.MGPLL5_ENABLE,
         ...
         Registers.DKL_PLL_COLD_BIAS_5),
      TCPLL6 =>
        (Registers.MGPLL6_ENABLE,
   ...
   is
      ...
      Clock_Khz : constant Frequency_Type := Clock / 1_000;
   begin
      if Display = DP then
         DCO_Min_Freq := 8_100_000;
         DCO_Max_Freq := 8_100_000;
      else
         DCO_Min_Freq := 7_992_000;
         DCO_Max_Freq := 10_000_000;
      end if;
      ...
      for I in Dividers'Range loop
         for Div2 in reverse 1 .. 10 loop
            DCO_Khz := Dividers (I) * Pos64 (Div2) * Pos64 (Clock_Khz) * 5;

            if DCO_Khz >= DCO_Min_Freq and then DCO_Khz <= DCO_Max_Freq then
               if Div2 >= 2 then
   ...
   begin
      ...
      Registers.Set_Mask
        (Register => PLL_Regs (PLL).DKL_PLL_ENABLE,
      ...
      declare
         ...
         Refclk_Khz : Frequency_Type;
         FeedFwGain : Word32;
         M1Div : constant := 2;
         M2Div_Int, M2Div_Rem : Word32;
         M2Div_Frac : Word32;
         TDC_Target : Word32;
      ...
      begin
         ...
         Refclk_Khz := Frequency_Type (Refclk / 1_000);
         M2Div_Int := Word32 (DCO_Khz / (Refclk_Khz * M1Div));
         M2Div_Rem := Word32 (DCO_Khz rem (Refclk_Khz * M1Div));
         Tmp := Word64 (M2Div_Rem) * 2 ** 22;
         M2Div_Frac := Word32 (Tmp / Word64 (Refclk_Khz * M1Div));
