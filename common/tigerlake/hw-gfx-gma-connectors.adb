...
package body HW.GFX.GMA.Connectors is
   ...
   is
      ...
      Clk_Sel_Shift : constant Natural := DDI_CLK_SEL_SHIFT (Port);
   begin
   ...
   begin
      ...
      --if Port in USBC_Port then
      --   TC.Set_Lane_Count (Port, Natural (Lane_Count_As_Integer (Port_Cfg.DP.Lane_Count)));
      --end if;
      ...
      if Port_Cfg.Display = DP then
         ...
         if Port in Combo_Port then
            Combo_Phy.Enable_HDMI (Port, Port_Cfg.Mode.Dotclock);
         else
            TC.Enable_HDMI (Port);
         end if;
   ...
   begin
      ...
      if Port_Cfg.Display = HDMI then
         if Port_Cfg.Port in Combo_Port then
            Combo_Phy.Enable_HDMI (Port_Cfg.Port, Port_Cfg.Mode.Dotclock);
         else
