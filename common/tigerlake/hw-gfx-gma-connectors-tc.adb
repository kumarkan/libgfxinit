...
package body HW.GFX.GMA.Connectors.TC is
   ...
   begin
      ...
      Lane_Count := (if Lanes < Lane_Count then Lanes else Lane_Count);

      Registers.Unset_And_Set_Mask
        (Register   => Fia_Regs (Port).PORT_TX_DFLEXDPMLE1,
	 Mask_Unset => DFLEXDPMLE1_DPMLETC_MASK (Port),
	 Mask_Set   => (case Lane_Count is
                        when 1 => DFLEXDPMLE1_DPMLETC_ML0 (Port), -- ML0 is not lane-reversed, ML3 is reverse
                        when 2 => DFLEXDPMLE1_DPMLETC_ML1_0 (Port), -- ML1_0 is not reversed, ML3_2 is reverse
